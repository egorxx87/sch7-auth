...
function applyHolidayUI_() {
  ...
  /***********************
   * LANE NORMALIZATION
   * Fixes "shifting columns" when the sheet puts the same person into different slots during the day.
   * Rule: once a person got a lane (slot) for a role+day, they keep it for the whole day.
   ***********************/
  function uniqNames_(arr) {
    const out = [];
    const seen = new Set();
    (arr || []).forEach(v => {
  ...
  function normalizeDayRowsForRoleLanes_(rows, roleKey, cols) {
    const nameToLane = new Map(); // persistent for this day+role
    return rows.map(r => {
      const orig = (r[roleKey] || []).slice(0, cols);
      while (orig.length < cols) orig.push("");
      const active = uniqNames_(orig);

      const laneArr = new Array(cols).fill("");
      const used = new Set();

      // 1) keep existing lane mapping if possible
      const unassigned = [];
      active.forEach(name => {
        const lane = nameToLane.has(name) ? nameToLane.get(name) : null;
        if (lane !== null && lane >= 0 && lane < cols && !used.has(lane)) {
          laneArr[lane] = name;
          used.add(lane);
        } else {
      ...
      // 2) assign remaining to first free lane (stable order)
      unassigned.forEach(name => {
        let lane = 0;
        while (lane < cols && used.has(lane)) lane++;
        if (lane >= cols) return; // no space, drop (shouldn't happen with normal data)
        laneArr[lane] = name;
        used.add(lane);
        nameToLane.set(name, lane);
      });

      // also set mapping for those that already had lane but were assigned now
      laneArr.forEach((name, lane) => { if (name) nameToLane.set(name, lane); });

      // return cloned row with normalized lanes
  ...
  function normalizeDayRowsForAllRoles_(rows, roles) {
    let out = rows.map(r => Object.assign({}, r)); // shallow clone
    roles.forEach(role => {
      out = normalizeDayRowsForRoleLanes_(out, role.key, role.cols);
    });
    return out;
  }
...
// ✅ (2) allRows уже собран как union всех закешированных месяцев внутри ensureMonthsLoaded_
  ...
  function renderDayGridCells(dayRows, targetEl = scheduleContentEl, roleFilter = "all") {
    let rows = dayRows.slice().sort((a, b) => (a.time || "").localeCompare(b.time || ""));
    ...
    // normalize lanes so the same person stays in the same slot for the whole day
    rows = normalizeDayRowsForAllRoles_(rows, roles);
...
function renderWeekRoleTimeline(roleKey) {
  ...
  /*****************************************************************
   * ✅ FIX: стабільні "під-колонки" у тижневому режимі по ролі
   * Правило: якщо ім'я хоч раз зайняло слот у цей день — воно
   * тримає цей слот до кінця дня (не "стрибає" у 1-й вільний).
   *****************************************************************/
  const byDate = new Map();
  currentRows.forEach(r => {
    if (!r.date) return;
  ...
  let normalizedRows = [];
  for (const [iso, rows] of byDate.entries()) {
    const sorted = rows.slice().sort((a, b) => (a.time || "").localeCompare(b.time || ""));
    const norm = normalizeDayRowsForRoleLanes_(sorted, roleKey, roleCols);
    normalizedRows = normalizedRows.concat(norm);
  }

  // map: "YYYY-MM-DD|HH:MM" -> rowObj (з нормалізованими слотами)
  const map = new Map();
  ...
  function extendShift(name, startRowNum, role, slot, hoursToExtend) {
    ...
    for (let i = 1; i <= hoursToExtend; i++) {
      ...
      if (target[role] && Array.isArray(target[role])) {
        // clear duplicates of the same name in other slots
        for (let s = 0; s < target[role].length; s++) {
          if (s === slot) continue;
          if (String(target[role][s] || "").trim() === name) {
            target[role][s] = "";
            postNoCors({ action: "schedule_update", row: target.row, role, slot: s, value: "" }).catch(console.error);
          }
        }
        if (typeof target[role][slot] !== "undefined") {
  ...
  function applySelection(name) {
    ...
    // If the same name exists in other slots in this row (because the sheet "shifted" it),
    // clear duplicates so UI+sheet becomes stable.
    if (trimmed && obj && obj[role] && Array.isArray(obj[role])) {
      for (let s = 0; s < obj[role].length; s++) {
        if (s === slot) continue;
        if (String(obj[role][s] || "").trim() === trimmed) {
          obj[role][s] = "";
          postNoCors({ action: "schedule_update", row, role, slot: s, value: "" }).catch(console.error);
        }
      }
    }

    // set selected lane(slot)
